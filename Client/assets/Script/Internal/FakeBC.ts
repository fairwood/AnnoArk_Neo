...
export default class FakeBC {

    private static _instance: FakeBC;

    public static get instance(): FakeBC {
        if (!FakeBC._instance) {
            FakeBC._instance = new FakeBC();
            FakeBC._instance.init();
        }
        return FakeBC._instance;
    }
    ...
    callFunction(from, func, args, value) {
        try {
            ...
            this.Blockchain.transaction.from = from;
            let result = this.gameContract[func](...argList);
            let resp = {};
            resp['result'] = result ? JSON.stringify(result) : null;
            return resp;
        } catch (e) {
            console.error(e);
        }
    ...
    init() {
        ...
        let LocalContractStorage = {
            defineProperty: function (root, key, arg2?) {
                root[key] = arg2;
            },
            defineMapProperty: function (root, key, arg2) {
                root[key] = arg2;
                root[key].get = function (key1) {
                    return root[key][key1] || null;
                }
                root[key].set = function (key1, value) {
                    root[key][key1] = value;
                }
            }
        };
        ...
        let GameContract = function () {
            ...
            LocalContractStorage.defineProperty(this, "totalNas");
        ...
        GameContract.prototype = {
            init: function () {
                this.adminAddress = 'APL5FCFSZrnG8L3cinkDRmXFDb27quJUWE';// Blockchain.transaction.from;
                this.totalNas = 0;
            ...
            claimNewUser: function (nickname, country) {
                ...
                this.totalNas = this.totalNas+(value);
            ...
            demolish: function (i, j) {
                ...
                if (building.money > 0) {
                    this._transaction(userAddress, (building.money) * ((1e18)));
                }
            ...
            attackIsland: function (islandIndex, army) {
                ...
                if (island.occupant !== "") {
                    ...
                    let hoursDelta = ((curTime - island.lastCalcTime)) / (1000 * 3600);
                    let attenu = Math.exp(powerAttenuRate * (hoursDelta) * (-1));
            ...
            _collectIslandMoneyInternal: function (island) {
                let curTime = (new Date()).valueOf();
                let hoursDelta = ((curTime - island.lastMineTime)) / (1000 * 3600);
            ...
            sponsor: function (islandIndex, sponsorName, link, pic) {
                ...
                if (island.sponsor !== userAddress) {
                    ...
                    this.totalNas = this.totalNas-(island.money);
            ...
            bancorTrade: function (cargoName, b) {
                ...
                if (b > 0) { //buy
                    ...
                    if (returnMoney > 1e-4) {
                        let returnWei = (Math.floor(returnMoney / 1e8) * 1e8);
                    ...
                    let money = (Math.floor(c * (1 - Fee) * 1e10) * 1e8);
            ...
            takeRedundantNas: function (targetAddress, value) {
                ...
                this.totalNas = this.totalNas-(value);
                this._transaction(targetAddress, (value));
                return {
                    "success": true,
                }
            },
